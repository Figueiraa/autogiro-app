...
flowchart TB
    ...
    subgraph eks["Amazon EKS · cluster gerenciado"]
        subgraph nskong["namespace kong"]
            KONG["Kong Gateway OSS<br/>DB-less · plugin jwt<br/>proxy: LoadBalancer (NLB público)<br/>admin: ClusterIP (não exposto)"]
        end
        ...
        NODES["Node group gerenciado<br/>2x t4g.small (Graviton)<br/>escala 2 a 4"]
    ...
    classDef pago fill:#e3f2fd,stroke:#1565c0,color:#0d47a1
    class aws,neon,nrcloud gratis
    class eks pago
```
...
| Cluster Kubernetes com escalabilidade | Amazon EKS + HPA (2 a 10) + metrics-server |
