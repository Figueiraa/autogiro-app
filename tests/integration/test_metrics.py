...
class TestEndpointDeMetricas:
    async def test_responde_200_no_formato_prometheus(self, client):
        ...
        assert "# TYPE autogiro_http_requests_total counter" in resp.text

    async def test_expoe_metadados_da_aplicacao(self, client):
        payload = await _scrape(client)

        assert _sample(payload, "autogiro_app_info", {"name": "autogiro-api"}) == 1

    async def test_endpoint_de_metricas_nao_se_instrumenta(self, client):
        await client.get("/metrics")
        payload = await _scrape(client)

        assert _sample(payload, "autogiro_http_requests_total", {"endpoint": "/metrics"}) == 0
...
class TestMetricasHttp:
    ...
    async def test_registra_duracao_da_requisicao(self, client):
        ...
        count = _sample(
            payload,
            "autogiro_http_request_duration_seconds_count",
    ...
    async def test_rota_inexistente_e_agrupada_em_unmatched(self, client):
        ...
        assert _sample(payload, "autogiro_http_requests_total", {"endpoint": "unmatched"}) >= 1
...
class TestMetricasDeNegocio:
    async def test_abertura_de_os_incrementa_contador(self, auth_client):
        antes = _sample(await _scrape(auth_client), "autogiro_service_orders_opened_total")

        await _criar_os(auth_client)

        depois = _sample(await _scrape(auth_client), "autogiro_service_orders_opened_total")
        assert depois == antes + 1
    ...
    async def test_aprovacao_de_orcamento_incrementa_contador(self, auth_client):
        ...
        antes = _sample(
            await _scrape(auth_client), "autogiro_budget_approvals_total", {"result": "approved"}
        )
    ...
    async def test_recusa_de_orcamento_incrementa_contador(self, auth_client):
        ...
        antes = _sample(
            await _scrape(auth_client), "autogiro_budget_approvals_total", {"result": "refused"}
        )
