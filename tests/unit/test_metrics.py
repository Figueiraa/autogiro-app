...
class TestRenderLatest:
    def test_expoe_texto_no_formato_prometheus(self):
        ...
        assert "# HELP autogiro_http_requests_total" in payload
        assert "# TYPE autogiro_http_requests_total counter" in payload
...
class TestAppInfo:
    def test_registra_nome_e_versao(self):
        metrics.set_app_info(name="autogiro-teste", version="9.9.9")
        payload = metrics.render_latest().decode("utf-8")

        assert (
            _sample(payload, "autogiro_app_info", {"name": "autogiro-teste", "version": "9.9.9"}) == 1
        )
