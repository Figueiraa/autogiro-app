...
monitoring/
├── prometheus/
│   ├── prometheus.yml          # scrape da API (job autogiro-api) a cada 15s
...
└── grafana/
    ...
    └── dashboards/
        └── autogiro-api.json   # dashboard "AutoGiro API — Observabilidade"
```
...
file* → substitua `grafana/dashboards/autogiro-api.json` e faça commit.
